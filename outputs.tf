...
output "tag_associations_summary" {
  ...
  value = var.enable_tagging && var.auto_apply_tags && var.create_tag_schema ? {
    ...
    message = "Auto-tagging is enabled and tags have been applied."
    } : {
    databases_tagged     = 0
    warehouses_tagged    = 0
    schemas_tagged       = 0
    roles_tagged         = 0
    service_users_tagged = 0
    central_db_tagged    = 0
    tags_applied         = []
...
output "custom_roles" {
  description = "Information about custom roles"
  value = var.enable_rbac ? {
    for role_name, role in snowflake_account_role.custom_roles :
    role_name => {
      id      = role.id
      name    = role.name
      comment = role.comment
...
output "functional_role_names" {
  description = "Names of functional roles (READER, WRITER, ADMIN)"
  value = var.enable_rbac && var.create_default_roles ? [
...
output "data_access_role_names" {
  description = "Names of data access roles (ALL_DATA, ANALYSIS_ONLY, INGEST_ONLY)"
  value = var.enable_rbac && var.create_default_roles ? [
...
output "custom_role_names" {
  description = "Names of custom roles"
  value = var.enable_rbac ? [
    for role in snowflake_account_role.custom_roles : role.name
  ] : []
}
...
output "all_role_names" {
  ...
  value = concat(
    ...
    var.enable_rbac ? [for role in snowflake_account_role.custom_roles : role.name] : []
  )
}
...
output "configuration_summary" {
  ...
  value = {
    ...
    resources_created = {
      ...
      functional_roles  = var.enable_rbac && var.create_default_roles ? 3 : 0 # READER, WRITER, ADMIN
      data_access_roles = var.enable_rbac && var.create_default_roles ? 3 : 0 # ALL_DATA, ANALYSIS_ONLY, INGEST_ONLY
      custom_roles      = var.enable_rbac ? length(var.custom_roles) : 0
