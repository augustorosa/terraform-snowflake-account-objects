...
graph TD
    ...
    subgraph "DATA ACCESS ROLES"
        ...
        ANALYSIS_ONLY[DEV_ANALYTICS_ANALYSIS_ONLY_ROLE]
...
|------|---------|-------------|-----------|------------|
| **ALL_DATA** | Access to all layers | RAW + PREPARE + ANALYSIS | Data engineers, Scientists | ADMIN |
| **ANALYSIS_ONLY** | Read-only analytics | ANALYSIS layer only | Business users, Analysts | ADMIN |
...
resource "snowflake_grant_account_role" "analyze_only_to_admin" {
  ...
  role_name        = "${local.base_prefix}_ANALYSIS_ONLY_ROLE"
  parent_role_name = "${local.base_prefix}_ADMIN_ROLE"
}
...
graph LR
    subgraph "DATABASE: DEV_ANALYTICS_DB"
        ...
        subgraph "ANALYSIS SCHEMA"
            ANALYSIS_TABLES[Metrics Tables]
            ANALYSIS_VIEWS[Dashboard Views]
        end
    end
    ...
    subgraph "DATA ACCESS"
        ...
        ANALYSIS_ONLY[ANALYSIS_ONLY_ROLE]
        INGEST_ONLY[INGEST_ONLY_ROLE]
    end

    READER --> ANALYSIS_ONLY
    WRITER --> ALL_DATA
    ADMIN --> ALL_DATA
    ADMIN --> INGEST_ONLY
    ...
    ALL_DATA --> ANALYSIS_TABLES
    
    ANALYSIS_ONLY --> ANALYSIS_TABLES
    ANALYSIS_ONLY --> ANALYSIS_VIEWS
...
| Role | RAW Schema | PREPARE Schema | ANALYSIS Schema | Inherits From |
|------|------------|----------------|----------------|---------------|
