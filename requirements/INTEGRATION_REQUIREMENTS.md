...
### 2.1 Purpose
dbt (data build tool) is used for transforming data within Snowflake, primarily operating between the RAW → PREPARE and PREPARE → ANALYZE layers.
...
| Schema (ANALYZE) | USAGE, CREATE TABLE, CREATE VIEW | Create models |
...
with DAG('data_pipeline', ...) as dag:
    ...
    # ANALYZE layer: Update analytics
