...
│   ├── Database Module
│   │   ├── Database Submodule
│   │   └── Schema Submodule (RAW/PREPARE/ANALYZE)
│   │
│   ├── Warehouse Module
...
| Role | RAW Layer | PREPARE Layer | ANALYZE Layer | Warehouses |
|------|-----------|---------------|---------------|------------|
...
#### 5.2.3 ANALYZE Schemas
```sql
-- Naming: {business_function}_{type}
CREATE SCHEMA ANALYZE.SALES_METRICS;
CREATE SCHEMA ANALYZE.CUSTOMER_DIMENSIONS;
CREATE SCHEMA ANALYZE.EXECUTIVE_REPORTS;
```
...
```
External Sources → Ingestion Tools → RAW → Transformation Tools → PREPARE → Analytics Tools → ANALYZE → BI/Reporting
```
...
- **Target**: PREPARE and ANALYZE layers
